...
export class UserService {
  ...
  getUserCredentials(): Observable<User | null> {
    ...
    return this.http.get<User>(this.baseUrl + "/me", { headers: header }).pipe(
  ...
  createUser(newUser: NewUser): Observable<{ message: string } | null> {
    return this.http
      .post<{ message: string }>(this.baseUrl + "/register", newUser)
      .pipe(
        catchError((err: HttpErrorResponse) => {
          if (err.status == 400) {
  ...
  resetPasswordConfirm(token: string, password: string) {
    return this.http
      .post(this.baseUrl + "/reset_password/confirm", {
        token: token,
        password: password,
      })
      .pipe(
  ...
  login(email: string, password: string) {
    return this.http
      .post<{ token: string; refresh_token: string }>(this.baseUrl + "/login", {
        email,
        password,
      })
      .pipe(
  ...
  editCredentials(credentials: UpdateCredentials) {
    ...
    const header = new HttpHeaders().set("Authorization", `JWT${token}`);
    return this.http
      .patch(this.baseUrl + "/me", credentials, { headers: header })
      .pipe(
        catchError((err: HttpErrorResponse) => {
          if (err.status == 400) {
  ...
  getMyProducts(
    ...
    if (itemsFilters) {
      ...
      if (itemsFilters.orderBy)
        params = params.set("order_by", itemsFilters.orderBy);
      if (itemsFilters.category)
        params = params.set("category", itemsFilters.category);
      if (itemsFilters.search)
        params = params.set("search", itemsFilters.search);
      if (itemsFilters.type) params = params.set("type", itemsFilters.type);
    }
    return this.http
      .get<PaginatedResponse<Item>>(this.baseUrl + "/me/items", {
        params: params,
        headers: header,
      })
      .pipe(
  ...
  changePassword(oldPassword: string, newPassword: string) {
    ...
    return this.http.post(
      this.baseUrl + "/me/change_password",
      {
  ...
  loginAttempts(
    ...
    return this.http
      .get<PaginatedResponse<LoginAttempt>>(
        this.baseUrl + "/me/login_attempts",
  ...
  checkGeneratedReport(): Observable<{
    is_completed: 1 | 0;
    created_at: string;
  }> {
    const token = this.authService.getToken();
    const header = new HttpHeaders().set("Authorization", `JWT ${token}`);
    return this.http.get<{ is_completed: 1; created_at: string }>(
      this.baseUrl + "/me/auctions_report",
      { headers: header }
    );
  }
  ...
  refreshAccessToken(
    refreshToken: string
  ): Observable<{ token: string; refresh_token: string }> {
