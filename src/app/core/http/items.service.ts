...
export class ItemsService {
  ...
  getItems(itemsFilters?: ItemsFilters): Observable<PaginatedResponse<Item>> {
    ...
    return this.http
      .get<PaginatedResponse<Item>>(this.baseUrl, { params: params })
      .pipe(
  ...
  createNewItem(newItem: NewItem) {
    ...
    return this.http.post<{ message: string; id_item: number }>(
      this.baseUrl,
      {
        name: newItem.name,
        description: newItem.description,
        id_category: newItem.category,
        starting_price: newItem.startingPrice,
  ...
  addImagesToItem(idItem: number, image: File, isMain: string) {
    ...
    return this.http.post<{ message: string }>(this.itemImagesUrl, formdata, {
      headers: header,
    });
  }
}
