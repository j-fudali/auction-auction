...
@Component({
  ...
  template: `
    <div
      class="container"
      *ngIf="{ isLtSm: (isLtSm$ | async) } as bp"
      [style.fontSize]="bp.isLtSm ? '12px' : '16px'"
      [style.padding]="bp.isLtSm ? '10px 10px' : '10px 75px'"
    >
        ...
        <ng-container matColumnDef="isSuccessful">
          ...
          <td
            ...
            *matCellDef="let element"
            [style.color]="element.is_successful ? 'green' : 'red'"
          >
            {{ element.is_successful === 1 ? "Yes" : "No" }}
          </td>
        </ng-container>
        <tr
          mat-header-row
          *matHeaderRowDef="bp.isLtSm ? onSmallColumns : displayedColumns"
        ></tr>
        <tr
          mat-row
          *matRowDef="
            let row;
            columns: bp.isLtSm ? onSmallColumns : displayedColumns
          "
        ></tr>
      </table>
      <mat-paginator
...
export class LoginHistoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.dataSource$ = this.userService.loginAttempts().pipe(
      tap((res) => {
  ...
  loadLoginAttempts(page?: number, orderBy?: string) {
    return this.userService.loginAttempts(page, orderBy).pipe(
      tap((res) => {
        this.itemsCount = res.items_count;
      }),
      map((res) => res.result)
